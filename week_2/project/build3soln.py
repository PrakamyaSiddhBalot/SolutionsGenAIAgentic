...
class ChatApp(App):
    ...
    # Note:
    # Ctrl+K may be intercepted by some terminals on Windows, as it was being intercepted on my terminal using Ctrl+K. 
    # Tested successfully using F6 as a temporary substitute. I even used ctrl+r and that's working fine too. So the problem seems to be that ctrl+k is reserved by the terminal and is therefore not being used inside the ChatApp.
